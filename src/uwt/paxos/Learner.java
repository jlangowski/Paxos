...
public class Learner {
	...
	public JSONObject isLocked(int lock) throws JSONException {
		...
		if (locks[lock] != null) {
			response.put("locked", true);// = "{\"locked\":true, \"clientId\":" + locks[lock]+"}";
			response.put("clientId", locks[lock]);
		} else {
			response.put("locked", false); // "{\"locked\":false}";
		}
		return response;
	}
	...
	public JSONObject lock(int lock, int clientId) throws JSONException {
		JSONObject response = new JSONObject();
		if (locks[lock] != null) {
			response.put("locked", false);// = "{\"locked\":false, \"clientId\":" + locks[lock]+"}";
			response.put("clientId", locks[lock]);
		} else {
			locks[lock] = new Integer(clientId);
			response.put("locked", true); // "{\"locked\":true}";
		}
		return response;
	}
		...
		while (true) {
			try  { 
				System.out.println("wating for connection");
				    ...
				    JSONObject request = new JSONObject( (String)is.readObject());
